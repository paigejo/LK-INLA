...
if( runMode=="parallel"){
  library(doParallel)
  library(foreach)
  source("heaton/heatoncomparison-master/Code/LatticeKrig/LKrig.sim.conditional.foreach.R")
}
...
if( dataCase=="simulated"){
  load("heaton/heatoncomparison-master/Data/SimulatedTemps.RData")
  lon <- sim.data$Lon
  lat <- sim.data$Lat
  temps <- sim.data$Temp
  NC<- 30
...
if( dataCase=="SatelliteTemps"){
  load("heaton/heatoncomparison-master/Data/SatelliteTemps.RData")
  lon <- sat.temps$Lon
  lat <- sat.temps$Lat
...
########################################################
  ...
  save( finalResults, fitFinal, comp.time, file=paste0("savedOutput/heaton/", dataCase,"NC",NC,"nlevel",nlevel,"finalResults.rda") )
...
##################################
  if( plotting=="yes"){
  pdf(paste0("Figures/applicationHeaton/", dataCase,"NC",NC,"nlevel",nlevel,"SummaryFitLK.pdf"), width=6, height=8)
