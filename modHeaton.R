...
modHeaton = function(dat, obsCoords=cbind(dat$Lon, dat$Lat), obsValues=dat$MaskTemp, 
                     nonlinearVarName="elev", 
                     xObs=cbind(1, dat$Lon, dat$Lat, dat[[nonlinearVarName]]), predNonlinearVar, 
                     predCoords, xPred=cbind(1, predCoords, predNonlinearVar), latInfo=NULL, 
  ...
  if(is.null(latInfo)) {
    ...
    for(i in 1:length(latInfo)) {
      ...
      latInfo[[i]]$xRangeDat = range(dat$Lon)
      latInfo[[i]]$yRangeDat = range(dat$Lat)
    }
  }
  ...
  if(is.null(rwKnots)) {
    rwKnots = make_knots(c(dat[[nonlinearVarName]], predNonlinearVar), n=nNonlinearBasis)[[1]]
  }
  
  # must set each value of the covariates to the closest knot
  rwEffects = match_with_knots(dat[[nonlinearVarName]], rwKnots)
  rwEffectsNew = match_with_knots(predNonlinearVar, rwKnots)
